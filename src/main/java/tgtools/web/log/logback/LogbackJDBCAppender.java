 ...
 * logback 数据库日志
 */
public class LogbackJDBCAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    protected void closeConnection(Connection con) {
        try {
            ...
            this.addError("Error closing connection", e);
        }
    }
    ...
    private boolean checkTable() {
        ...
        try {
            ...
            if (null != conn) {
                ...
                System.out.println("日志表模式：" + (m_IsPlatForm ? "平台模式1" : "普通模式"));
                m_hasTable = true;
                return true;
            }
        } catch (Exception e) {
    ...
    protected void append(ILoggingEvent iLoggingEvent) {
        ...
        if(null==iLoggingEvent.getArgumentArray())
        {
            return ;
        }
        if (iLoggingEvent.getArgumentArray().length < 1 || !(iLoggingEvent.getArgumentArray()[0] instanceof LogEntity)) {
            return;
        }
        ...
        try {
            LogEntity entity = (LogEntity) iLoggingEvent.getArgumentArray()[0];
            conn = getConnection();
            if (null == conn) {
                return;
            }
            ...
            this.addError("log into database failed!", ex);
        } finally {
            try {
                pstmt.close();
